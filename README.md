...
### V2.1.0 (2024-12-27) — 当前版本

**新增功能**

- ➕ 统计数据查询接口 `ring_buffer_get_stats()`
- ➕ 自定义策略注销功能 `ring_buffer_unregister_ops()`
- ➕ 完整的单元测试套件 `ring_buffer_test.c`

**优化改进**

- ⚡ 日志宏定义重构,减少 80% 代码重复
- 📄 统一版本号管理,移除单文件版本号
- 📖 README 文档重写,增加详细使用示例

**缺陷修复**

- 🐛 修复环绕边界条件判断错误
- 🐛 修复 `write_multi()` 部分写入统计错误

------

### V2.0.0 (2024-12-20) — 重大架构升级 ⚠️
...
err = ring_buffer_init(&rb, buffer, 256);
if (err != RB_OK) { /* 错误处理 */ }

/* v2.0 新代码 */
ring_buffer_t rb;
if (!ring_buffer_create(&rb, buffer, 256, RING_BUFFER_TYPE_LOCKFREE)) {
    /* 错误处理 */
}
```
